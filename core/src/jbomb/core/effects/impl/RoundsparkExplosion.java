package jbomb.core.effects.impl;

import com.jme3.math.ColorRGBA;
import com.jme3.math.Vector3f;


public class RoundsparkExplosion extends BaseExplosion {
    public RoundsparkExplosion() {
        super("roundsparkExplosion", 1, "roundspark.png");
    }

    @Override
    public void config() {
        emitter.setStartColor(ColorRGBA.White);
        emitter.setEndColor(ColorRGBA.White);
        emitter.setStartSize(3f);
        emitter.setEndSize(6f);
        emitter.setRandomAngle(true);
        emitter.getParticleInfluencer().setInitialVelocity(Vector3f.UNIT_Y);
        emitter.getParticleInfluencer().setVelocityVariation(1f);
    }

    @Override
    public float getTimeForDie() {
        return 3.2f;
    }
}
